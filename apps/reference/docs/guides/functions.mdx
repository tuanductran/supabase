...
Edge Functions supports `GET`, `POST`, `PUT`, `PATCH`, and `DELETE`. A function can be designed to perform different actions based on request's HTTP method. See the [example on building a RESTful service](https://github.com/supabase/supabase/tree/master/examples/edge-functions/supabase/functions/restful-tasks) to learn how to handle different HTTP methods in your function.
