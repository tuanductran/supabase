...
const InfraLayout: FC<Props> = ({ children }) => {
  const { ui } = useStore()
    ...
    <BaseLayout hideHeader hideIconBar>
      <div className="flex h-full w-full flex-col">
        ...
        <div className="flex items-center space-x-4 border-b py-4 px-5 dark:border-dark">
          <Link href={`/project/${ui.selectedProject?.ref}/settings/infra`} passHref>
            <a className="text-scale-900 transition-colors hover:text-scale-1200">
              <IconX size={16} strokeWidth={1.5} />
            </a>
          </Link>
          <div className="flex items-center space-x-6">
            <h1 className="text-sm text-scale-1200">Customize your project</h1>
...
export default withAuth(observer(InfraLayout))
